...
func (a *abb[K, V]) iterarWraper(desde *K, hasta *K, visitar func(clave K, dato V) bool, nodo *nodo[K, V]) {
	if nodo == nil {
		return
	}

	if desde != nil && a.comparacion(nodo.clave, *desde) <= 0 {
		a.iterarWraper(desde, hasta, visitar, nodo.hijo_der)
		return
	}
	if hasta != nil && a.comparacion(nodo.clave, *hasta) >= 0 {
		a.iterarWraper(desde, hasta, visitar, nodo.hijo_izq)
		return
	}
	//magia
